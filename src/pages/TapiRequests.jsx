...
function genRequestId(requests) {
  if (!requests || requests.length === 0) return 'request_01'
  ...
  return `request_${String(next).padStart(2, '0')}`
}
...
export default function TapiRequests() {
  ...
  const statusOf = (val) => STATUS_OPTIONS.find(s => s.value === val) || STATUS_OPTIONS[0]
    ...
    <div style={{ padding: 24, direction: 'rtl' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#888', marginTop: 3 }}>{requests.length} בקשות סה"כ</div>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', color: '#aaa', padding: 60, fontSize: 14 }}>
          אין בקשות עדיין — לחץ על "פתיחת בקשה" להתחיל
        </div>
      ) : (
...
function RequestModal({ onSave, onClose, nextId }) {
  ...
  async function handleSave() {
    if (!requester) {
      setError('יש לבחור שם מבקש הבקשה לפני השמירה')
      return
    }
    if (!text.trim()) {
      setError('יש להזין תוכן לבקשה')
      return
    }
    setSaving(true)
          ...
          <div ref={dropdownRef} style={{ position: 'relative' }}>
            <button
              ...
              style={{
                ...
                color: requester ? '#185FA5' : '#aaa',
                fontWeight: requester ? 600 : 400,
