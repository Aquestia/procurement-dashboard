...
const STATUS_OPTIONS = [
  { value: '—',       label: '—',       color: 'var(--text-muted)',    bg: 'var(--bg-neutral)' },
  ...
  return (
    <div style={{ padding: 24, direction: 'rtl' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16 }}>
        <div>
          <h1 style={{ fontSize: 18, fontWeight: 600, color: 'var(--text-main)', margin: 0 }}>בקשות רכש תפ"י</h1>
      ...
      {/* ── Filter bar ── */}
      <div style={{
        background: 'var(--bg-card)', border: '1px solid #e5e5e0', borderRadius: 10,
        padding: '12px 16px', marginBottom: 14,
        display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap',
      }}>
        {/* מבקש */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <label style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600 }}>מבקש</label>
          <select value={filterRequester} onChange={e => setFilterRequester(e.target.value)}
            style={{ fontSize: 12, padding: '5px 8px', borderRadius: 6, border: '1px solid var(--border-light)', background: '#fafaf8', cursor: 'pointer', minWidth: 130 }}>
        ...
        {/* סטטוס */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <label style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600 }}>סטטוס</label>
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
            style={{ fontSize: 12, padding: '5px 8px', borderRadius: 6, border: '1px solid var(--border-light)', background: '#fafaf8', cursor: 'pointer', minWidth: 100 }}>
            <option value="הכל">הכל</option>
        ...
        {/* תאריך מ */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <label style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600 }}>מתאריך</label>
          <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
            style={{ fontSize: 12, padding: '5px 8px', borderRadius: 6, border: '1px solid var(--border-light)', background: '#fafaf8', cursor: 'pointer' }} />
        </div>

        {/* תאריך עד */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <label style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600 }}>עד תאריך</label>
        ...
        {hasFilters && (
          <button onClick={clearFilters} style={{
            ...
            borderRadius: 6, border: '1px solid var(--border-light)', background: 'transparent',
            color: 'var(--text-muted)', cursor: 'pointer', marginBottom: 1,
          }}>✕ נקה סינון</button>
        )}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', color: 'var(--text-hint)', padding: 60, fontSize: 14 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: 'var(--bg-page)' }}>
                <th style={{ padding: '10px 14px', fontWeight: 600, fontSize: 11, color: 'var(--text-sub)', borderBottom: '1px solid #e0e0da', textAlign: 'right', whiteSpace: 'nowrap' }}>
                  <button onClick={() => setSortAsc(s => !s)} style={{
                    display: 'flex', alignItems: 'center', gap: 4,
                    background: 'none', border: 'none', cursor: 'pointer',
                    fontSize: 11, fontWeight: 600, color: 'var(--blue)', padding: 0,
                  }}>
                ...
                {['תאריך פתיחה','מבקש','תוכן הבקשה','סטטוס'].map(h => (
                  <th key={h} style={{
                    ...
                    color: 'var(--text-sub)', borderBottom: '1px solid #e0e0da',
            ...
            <tbody>
              {filtered.map((r, i) => (
                <tr key={r.id} style={{ background: i % 2 === 0 ? 'var(--bg-card)' : 'var(--bg-row)', verticalAlign: 'top' }}>
                  {/* מס' בקשה */}
                  <td style={{ padding: '10px 14px', borderBottom: '1px solid #f0f0ea', whiteSpace: 'nowrap' }}>
                    <span style={{
                  ...
                  {/* תאריך */}
                  <td style={{ padding: '10px 14px', borderBottom: '1px solid #f0f0ea', whiteSpace: 'nowrap', color: 'var(--text-sub)', fontSize: 11 }}>
                    {fmtDateTime(r.created_at)}
                  </td>
                  {/* מבקש */}
                  <td style={{ padding: '10px 14px', borderBottom: '1px solid #f0f0ea', whiteSpace: 'nowrap', fontWeight: 600 }}>
                    {r.requester}
                  </td>
                  {/* תוכן */}
                  <td style={{ padding: '10px 14px', borderBottom: '1px solid #f0f0ea', maxWidth: 500, lineHeight: 1.6, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                    {r.body}
                  </td>
                  {/* סטטוס */}
                  <td style={{ padding: '10px 14px', borderBottom: '1px solid #f0f0ea', whiteSpace: 'nowrap' }}>
...
function RequestModal({ onSave, onClose, nextId }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      onClick={onClose}>
      <div style={{ background: 'var(--bg-card)', borderRadius: 14, padding: 28, width: 620, maxHeight: '90vh', overflow: 'auto', direction: 'rtl', boxShadow: '0 8px 40px rgba(0,0,0,0.18)' }}
        onClick={e => e.stopPropagation()}>

        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-main)' }}>פתיחת בקשה חדשה</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, fontFamily: 'monospace' }}>{nextId}</div>
          </div>
          <button onClick={onClose} style={{ marginRight: 'auto', fontSize: 18, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>✕</button>
        </div>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <div ref={dropdownRef} style={{ position: 'relative' }}>
            <button onClick={() => setDropdownOpen(o => !o)} style={{
              ...
              background: requester ? 'var(--blue-bg)' : 'var(--bg-row)',
              color: requester ? '#185FA5' : '#aaa', fontWeight: requester ? 600 : 400,
              cursor: 'pointer', textAlign: 'right',
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            }}>
              <span>{requester || 'בחר שם...'}</span>
              <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>{dropdownOpen ? '▲' : '▼'}</span>
            </button>
            {dropdownOpen && (
              <div style={{
                position: 'absolute', top: '100%', right: 0, left: 0, zIndex: 10,
                background: 'var(--bg-card)', border: '1px solid #e0e0da', borderRadius: 8,
        ...
        <div style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: 8, gap: 8 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--blue)' }}>תוכן הבקשה *</span>
            <div style={{ display: 'flex', gap: 4, marginRight: 'auto' }}>
              ...
              <button title='מספור' onClick={() => { setText(t => t + (t && !t.endsWith('\n') ? '\n1. ' : '1. ')); setTimeout(() => textareaRef.current?.focus(), 0) }}
                style={{ fontSize:12, width:24, height:24, border:'1px solid var(--border-light)', borderRadius:4, background:'var(--bg-page)', cursor:'pointer' }}>1.</button>
            </div>
          </div>
          <textarea ref={textareaRef} value={text} onChange={e => { setText(e.target.value); setError('') }}
            ...
            style={{
              ...
              borderRadius:8, resize:'vertical', background:'var(--bg-row)',
              color:'var(--text-main)', lineHeight:1.7, fontFamily:'inherit',
              direction:'rtl', textAlign:'right', outline:'none', boxSizing:'border-box',
            }} />
        </div>

        {error && (
          <div style={{ marginBottom:14, padding:'8px 12px', background:'var(--red-bg)', border:'1px solid #F09595', borderRadius:7, fontSize:12, color:'var(--red-dark)', fontWeight:600 }}>
            ⚠️ {error}
          </div>
        )}
        ...
        <div style={{ display:'flex', gap:8, justifyContent:'flex-start' }}>
          <button onClick={handleSave} disabled={saving} style={{
            ...
            background: saving ? 'var(--text-hint)' : 'var(--blue)', color:'#fff', cursor: saving ? 'default' : 'pointer', fontWeight:600,
