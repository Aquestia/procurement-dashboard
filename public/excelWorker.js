...
function processExcelFile(buffer) {
  ...
  // Add BO items that don't appear in Calc shortage at all
  const boOnlyItems = buildBOOnlyItems(boSheet, shortagesWithBO, openOrders, soByPRD, dr4ByProd, dr5ByProd, dr4ByMain, dr5ByMain, soBySOItem, soBySOLine, isFormatB, poByItem)
  
  return [{ __meta: true, stageSummary, financials }, ...shortagesWithBO, ...boOnlyItems]
}

function buildBOOnlyItems(boRows, existingShortages, openOrders, soByPRD, dr4ByProd, dr5ByProd, dr4ByMain, dr5ByMain, soBySOItem, soBySOLine, isFormatB, poByItem) {
  // Items already in shortages
  const existingItems = new Set(existingShortages.map(r => str(r.itemNumber)))
  
  // Group BO rows by item
  const boByItem = {}
  boRows.forEach(r => {
    const item = str(r['Item Code']).trim()
    if (!item || existingItems.has(item)) return
    if (!boByItem[item]) boByItem[item] = []
    boByItem[item].push(r)
  })
  
  const results = []
  
  Object.entries(boByItem).forEach(([itemNumber, rows]) => {
    // Find orders from Sales
    const allOrders = []
    ...
    rows.forEach(r => {
      ...
      // Look up in soBySOLine or soBySOItem
      let orderInfo = soBySOLine[`${doc}-${line}`]
      if (!orderInfo) {
        const matches = soBySOItem[`${doc}__${itemNumber}`] || []
      ...
      if (!allKeys.has(key)) {
        ...
        allOrders.push({
          salesOrder: doc,
          lineNumber: line,
          customerName: orderInfo?.customerName || str(r['Customer'] || ''),
          confirmedShipDate: orderInfo?.confirmedShipDate || null,
          requestedShipDate: orderInfo?.requestedShipDate || null,
          remainingAmount: num(r['Back Orders $']),
          salesRemainingAmount: orderInfo?.salesRemainingAmount || num(r['Back Orders $']),
          isBO: true,
    ...
    const boAmount = rows.reduce((s, r) => s + num(r['Back Orders $']), 0)
    const pos = poByItem[itemNumber] || []
    const hasPO = pos.length > 0
    const hasNoDate = hasPO && pos.every(p => !p.confirmedReceiptDate)
    const sortedPOs = pos.filter(p => p.confirmedReceiptDate).sort((a,b) => new Date(a.confirmedReceiptDate) - new Date(b.confirmedReceiptDate))
    const vendors = [...new Set(pos.map(p => p.vendorName).filter(Boolean))]
    
    // Get product name from openOrders
    const productNameRow = openOrders.find(r => str(r['Item number']) === itemNumber)
    
    results.push({
      itemNumber,
      productName: str(productNameRow?.['Name'] || productNameRow?.['Product name'] || ''),
      isBO: true,
      procurementStatus: 'BO',
      stage: hasPO ? 'רכש' : 'לא ידוע',
      prd: allOrders[0]?.prd || '',
      rootPRDs: [],
      orders: allOrders,
      boOrders: allOrders,
      affectedOrdersCount: allOrders.length,
      boOrdersCount: allOrders.length,
      totalQtyRequired: 0,
      totalQtyPicked: 0,
      totalOnOrder: 0,
      totalAvailable: 0,
